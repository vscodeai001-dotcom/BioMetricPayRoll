...
public sealed class FirebaseSuperAdminProvisioningService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            _logger.LogError(ex, "SuperAdmin provisioning failed.");
        }
    }
    ...
    {
        if (!await _firebase.EnsureConfiguredAsync())
            return;
        ...
        {
            _logger.LogWarning(
                "Firebase Admin SDK is not initialized. Configure FIREBASE_SERVICE_ACCOUNT_JSON or GOOGLE_APPLICATION_CREDENTIALS.");
            return;
        }
        ...
        try
        {
            user = await auth.GetUserByEmailAsync(email, ct);
        }
            ...
            when (ex.AuthErrorCode == AuthErrorCode.UserNotFound)
        {
            ...
            {
                _logger.LogWarning(
                    "Firebase SuperAdmin {Email} does not exist. Set SUPERADMIN_PASSWORD once to provision it.",
                    email);
                return;
            }
        ...
        string? password)
    {
        ...
        if (user == null)
        {
            ...
            {
                _logger.LogWarning(
                    "Local SuperAdmin {Email} does not exist. Set SUPERADMIN_PASSWORD once to provision it.",
                    email);
                return;
            }
